...
export const generateP9APdf = async (req, res) => {
  ...
  if (!companyId || !employeeId || !year) {
    return res
      .status(400)
      .json({ error: "Company ID, Employee ID, and Year are required." });
  }
  ...
  try {
    ...
    // Filter out records where payroll_run is null
    const validPayrollData =
      payrollData?.filter((record) => record.payroll_run !== null) || [];

    if (validPayrollData.length === 0) {
      return res
        .status(404)
        .json({
          error: "P9A data not found for the specified employee and year.",
        });
    }

     // Security: ensure the first record belongs to the correct company
    const firstRecord = validPayrollData[0];
    ...
    const pdfBuffer = await generateP9APDF(
      validPayrollData,
      firstRecord.employee,
      firstRecord.payroll_run.company,
      year,
    );
  ...
  try {
    ...
    const { data: payrollData, error } = await supabase
     ...
     if (error) {
        ...
        // Filter out records where payroll_run is null
        const validPayrollData = payrollData?.filter(record => record.payroll_run !== null) || [];
   ...
   // Security: ensure the first record belongs to the correct company
    ...
    if (firstRecord.payroll_run.company.id !== companyId) {
      return res
        .status(403)
        .json({ error: "This P9A report does not belong to this company." });
    }
    ...
    if (!firstRecord.employee.email) {
      return res
        .status(400)
        .json({ error: "Employee email address not found." });
    }
    ...
    const employeeFullName =
      `${firstRecord.employee.first_name || ""} ${firstRecord.employee.last_name || ""}`.trim();
    // Assuming you'll create a new email template function for P9A
    const htmlContent = getP9AEmailTemplate(
      employeeFullName,
      firstRecord.payroll_run.company.business_name,
      year,
    );
