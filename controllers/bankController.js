...
export const generateBankDetailsTemplate = async (req, res) => {
    ...
    try {
        ...
        headerRow.eachCell((cell, colNumber) => {
            ...
            if (dropdownOptions[header] && dropdownOptions[header].length > 0) {
                worksheet.dataValidations.add(
                    `${worksheet.getColumn(colNumber).letter}2:${worksheet.getColumn(colNumber).letter}1000`,
                    {
                        type: 'list',
                        allowBlank: true,
        ...
        employees.forEach(employee => {
            const employeeName = `${employee.first_name} ${employee.last_name} ${employee.other_names || ''}`.trim();
            const rowData = [
        ...
        res.setHeader('Content-Disposition', 'attachment; filename=Employee_Bank_Details_Template.xlsx');
...
export const importBankDetails = async (req, res) => {
    ...
    try {
        ...
        if (companyError || !company) {
            return res.status(403).json({ error: 'Unauthorized to import bank details to this company.' });
        }
        ...
        const jsonData = utils.sheet_to_json(worksheet, { header: 1, raw: true, defval: null });
        ...
        for (let i = 1; i < jsonData.length; i++) {
            ...
            if (!employeeId) {
                errors.push(`Row ${i + 1}: Employee with number '${employeeNumber}' not found.`);
                continue;
            }

            const paymentMethod = bankData['payment_method']?.trim();
            if (!paymentMethod || !['Bank', 'M-Pesa', 'Cash'].includes(paymentMethod)) {
            ...
            const record = {
                ...
                bank_name: paymentMethod === 'Bank' ? (bankData['bank_name'] ? String(bankData['bank_name']).trim() : null) : null,
                bank_code: paymentMethod === 'Bank' ? (bankData['bank_code'] ? String(bankData['bank_code']).trim() : null) : null,
                account_number: paymentMethod === 'Bank' ? (bankData['account_number'] ? String(bankData['account_number']).trim() : null) : null,
                phone_number: paymentMethod === 'M-Pesa' ? (bankData['phone_number'] ? String(bankData['phone_number']).trim() : null) : null,
            };
        ...
        if (errors.length > 0) {
            return res.status(400).json({ error: "Import failed due to validation errors.", details: errors });
        }
        ...
        if (error) {
            ...
            return res.status(500).json({ error: "Failed to import employee bank details." });
        }
